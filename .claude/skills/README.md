### 前端代码vibe coding使用步骤


skills存放目录如下所示：

![image-20260515173956166](./images/image-20260515173956166.png)

#### 一、在项目下开启一个终端，并输入claude启动Claude Code

![image-20260515174318179](./images/image-20260515174318179.png)

#### 二、两种方式调用skills：

##### 1、直接输入/，选择/front-code-generator，根据提示粘贴文档路径

![image-20260518101633497](./images/image-20260518101633497.png)



##### 2、输入/选择要执行的skills，选择/front-code-generator ，按tab键，然后把需求文档绝对路径粘贴进来，再把事先准备好的接口文档绝对路径粘贴进来，enter执行即可。

```
/front-code-generator "C:\Users\heyue\Desktop\工作相关\v2.0.1\合同额预算评审\合同额预算评审.xlsx" "C:\Users\heyue\Desktop\工作相关\v2.0.1\合同额预算评审\合同额预算评审.md" 
```

