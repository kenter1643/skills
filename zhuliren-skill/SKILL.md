...
name: zhuliren
description: |
  主理人造人：输入人名/主题/甚至只是模糊需求，自动深度调研→思维框架提炼→生成可运行的人物Skill。
  内置「投资人 V2」专业分支，用于投资人、交易员、基金经理和资本配置者的证据化蒸馏与旧Skill升级。
  三种入口：(1)明确人名→直接蒸馏 (2)模糊需求→诊断推荐 (3)已有Skill→审计后增量迁移。
  触发词：「造skill」「蒸馏XX」「主理人」「投资人V2」「造人」「XX的思维方式」「做个XX视角」「更新XX的skill」。
...
当目标人物的主要能力来自投资、交易、基金管理或资本配置时，自动切换到**投资人 V2**。

投资人 V2 的目标不是模仿语气，而是复原并验证：
...
- `references/investor-v2-framework.md`：十维研究、证据等级和交付标准
- `references/investor-v2-template.md`：投资人物专用 Skill 模板
- `references/investor-v2-migration.md`：旧 Skill 审计与增量迁移

投资场景禁止把框架推演写成本人观点，禁止虚构实时持仓、成本、仓位和目标价。
...
6. **模式路由**：如果人物以投资、交易、基金管理或资本配置闻名，标记为 `investor-v2`；否则使用通用人物模式。
...
用户不知道该蒸馏谁，只有需求或困惑。这时主理人的工作是**从需求反推最合适的蒸馏对象**。
...
主理人：你说的决策主要是哪种场景？比如商业/投资决策，还是职业/人生方向的选择？

用户：主要是商业上的，比如要不要做某个产品、要不要接某个合作

主理人：明白了，你的核心需求是「在信息不完整时快速做出高质量的商业判断」。
我推荐3个候选：
[展示候选推荐...]
```
...
- `investor-v2` 新建项目使用十维研究结构，具体文件见 `references/investor-v2-framework.md`。
- 已有投资 Skill 不机械改名研究文件；先按 `references/investor-v2-migration.md` 审计，复用可靠内容后只补缺口。
...
- **通用人物模式**：继续使用下方6个Agent和六维研究文件。
- **投资人 V2**：读取 `references/investor-v2-framework.md`，按十维研究结构并行采集。必须覆盖估值、组合仓位、交易退出、成功案例、失败案例和市场环境。
- **旧投资 Skill 更新**：先运行 `python3 scripts/investor_v2_audit.py <skill目录>`，只对审计缺口启动Agent。
...
- 通用人物读取 `references/skill-template.md`。
- 投资人 V2 读取 `references/investor-v2-template.md`，不得套用“直接冒充本人”的角色扮演规则。
...
| 创建者归属 | 固定内容：`> 本Skill由 [主理人 · Skill造人术](https://github.com/kenter1643/skills/tree/main/zhuliren-skill) 生成` + `> 维护者：[kenter1643](https://github.com/kenter1643)；基于 [女娲.skill](https://github.com/alchaincyf/nuwa-skill) 修改` |

投资人 V2 不使用上表，按专用模板将十维研究映射到：投资身份卡、投资世界观、研究系统、估值与预期、组合仓位、买卖退出、环境适配、案例、失败和证据边界。
...
```bash
python3 scripts/quality_check.py --profile investment <SKILL.md路径>
```

投资专项门槛：独立来源、证据标签、三个案例、两个失败案例、仓位、退出、三条证伪条件、环境适配和数据截止日期。根据完整度分为 A 可实战调用、B 研究视角、C 人物理解。
...
1. 先判断是普通时效更新，还是迁移到投资人 V2。
2. 普通人物更新：读取现有SKILL.md，从「诚实边界」找到调研时间，只启动最新对话、最新决策和时间线Agent。
3. 投资人物更新：读取 `references/investor-v2-migration.md`，运行 `python3 scripts/investor_v2_audit.py <skill目录>`。
4. 对比新信息与现有内容：
   - 新信息强化现有模型 → 补充案例
...
5. 投资人物按审计结果选择迁移等级：
   ...
   - `M3`：仅当来源不可追溯或事实体系根本不可靠时重新蒸馏
6. 保留原 slug、触发词和可靠结论；不重写整个Skill，只增量更新。
7. 无法从公开资料确认的仓位、成本和动机写“未知”，不得靠推断补齐。
...
1. 主理人无法从公开渠道搜到用户的思维框架，需要用户提供素材
