...
<p align="center">
  <img src="assets/banner.svg" alt="主理人.skill" />
  <br/>
  <sub>人物思维框架蒸馏与 Skill 生成工具</sub>
</p>
...
**主理人帮你蒸馏任何人的思维方式，让乔布斯、马斯克、芒格、费曼都给你打工。**

**投资人 V2** 是主理人的投资专业分支：蒸馏投资人、交易员和基金经理时，不只提炼观点，
还复原研究、估值、组合仓位、买卖退出、失败案例和证伪条件。
...
只需输入一个名字，主理人自动完成调研、提炼、验证全流程。

[看效果](#效果示例) · [安装](#安装) · [投资人 V2](docs/investor-v2-guide.md) · [它蒸馏了什么](#主理人蒸馏了什么) · [工作原理](#工作原理) · [不想自己蒸馏？试试 Bloome](#不想自己蒸馏试试-bloome)
...
> 本项目是 [alchaincyf/nuwa-skill](https://github.com/alchaincyf/nuwa-skill) 的衍生版本，依据 MIT License 发布。原作者版权声明保留在 [LICENSE](LICENSE) 中；当前版本由 [kenter1643](https://github.com/kenter1643) 维护，并更名为“主理人”以便后续独立开发。
...
[<img src="bloome-advisors.jpg" alt="Bloome - 让他们成为你的智囊团" width="720">](https://www.bloome.im/?utm_source=huashu-leon-202605&utm_medium=github&utm_campaign=zhuliren-skill)

想随时找乔布斯和张小龙聊产品？和芒格聊投资？

[Bloome](https://www.bloome.im/?utm_source=huashu-leon-202605&utm_medium=github&utm_campaign=zhuliren-skill) —— 让他们成为你的智囊团，人和多个 Agent 在同一个对话里协作。[www.bloome.im](https://www.bloome.im/?utm_source=huashu-leon-202605&utm_medium=github&utm_campaign=zhuliren-skill)

---

## 安装

主理人基于开放的 [Agent Skills](https://agentskills.io) 协议，可在任何 skills-compatible 的 AI agent runtime 中运行。
...
```
帮我安装这个 skill：https://github.com/kenter1643/skills/tree/main/zhuliren-skill
```

或者用通用 CLI 安装器（[vercel-labs/skills](https://github.com/vercel-labs/skills)，支持 55+ runtime）：

```bash
npx skills add kenter1643/skills --skill zhuliren-skill
```
...
| Runtime | 安装路径 |
|---|---|
| Claude Code | `~/.claude/skills/zhuliren-skill/` |
| Codex CLI | `~/.codex/skills/zhuliren-skill/` |
| Cursor | `~/.cursor/skills/zhuliren-skill/` |
| OpenClaw | `~/.openclaw/workspace/skills/zhuliren-skill/` |
| Hermes Agent | 跑 `tools/install_hermes_skill.py` |
| 其他 runtime | clone 到对应 runtime 的 `skills/` 目录 |

```bash
git clone https://github.com/kenter1643/skills.git
cp -R skills/zhuliren-skill <上面对应的路径>
```
...
蒸馏各领域最强的人，需要提取比日常工作习惯更深的东西。主理人提取五层：
...
工作习惯可以靠流程文档传递，但让芒格和马斯克面对同一个问题做出不同判断的，是认知框架。主理人提取的是认知操作系统。
...
当目标人物的主要专业能力来自投资、交易、基金管理或资本配置时，主理人自动切换到
**投资人 V2**。它优先复原可执行、可验证的投资系统，而不是模仿人物语气：

```text
优势来源 → 研究流程 → 估值与预期 → 组合与仓位 → 买卖纪律 → 证伪与复盘
```

投资人 V2 使用十维研究结构，并给重要证据标注等级：

- `[P1]` 本人著作、股东信、演讲原文、监管原件
- `[P2]` 完整访谈、官方材料、公开持仓或交易披露
- `[S1/S2]` 权威二手资料与后世评论
- `[I]` 基于公开行为的推断
- `[U]` 未验证或存在争议，不进入核心模型
...
| 等级 | 用途 |
|---|---|
| **A 可实战调用** | 可用于具体标的分析，但必须补充最新数据 |
| **B 研究视角** | 可用于研究和反方审视，不输出具体仓位 |
| **C 人物理解** | 只用于理解人物，不用于投资决策 |

#### 已有 Skill 如何升级

旧 Skill 不需要全部重做。投资人 V2 先审计，再增量迁移：
...
- `M0` 无需迁移
- `M1` 只补结构、证据标签和回答协议
- `M2` 针对估值、仓位、退出、案例或失败等缺口补研
- `M3` 仅在来源不可追溯或事实体系不可靠时重新蒸馏

迁移保留原 slug、触发词和可靠结论；无法确认的仓位、成本、目标价和投资动机明确写“未知”。

完整的设计原则、十维研究结构、命令、交付等级和迁移样例见
**[《投资人 V2 详细使用说明》](docs/investor-v2-guide.md)**。

---

## 已蒸馏人物

主理人已蒸馏了13位人物 + 1个主题。每个都是独立的、可直接安装使用的Skill，全部基于 Agent Skills 协议，可在 Claude Code / Codex / Cursor / OpenClaw / Hermes 等 runtime 通用：
...
想蒸馏不在列表里的人或主题？安装主理人，说「蒸馏一个XXX」就行。
...
主理人造Skill，**[达尔文](https://github.com/alchaincyf/darwin-skill)** 让Skill进化。

受 Karpathy autoresearch 启发，达尔文.skill 用自主实验循环批量优化所有Skill：8维度评估、棘轮机制（只保留改进，自动回滚退步）、独立子agent评分。主理人的 Phase 5 双Agent精炼就内置了达尔文的评估体系，这也是主理人生成的Skill质量高的原因之一。
...
**1. 分模式并行采集**——通用人物使用六维研究；投资人 V2 使用十维研究，额外覆盖估值、组合仓位、交易退出、案例、失败和市场环境。
...
├── SKILL.md                      # 主理人本体
├── docs/
│   └── investor-v2-guide.md      # 投资人 V2 详细使用说明
├── references/
│   ├── extraction-framework.md   # 提炼方法论（想深入了解看这个）
│   ├── skill-template.md         # 通用人物模板
│   ├── investor-v2-framework.md  # 投资研究与证据协议
│   ├── investor-v2-template.md   # 投资人物模板
│   └── investor-v2-migration.md  # 旧Skill增量迁移协议
├── scripts/
│   ├── merge_research.py         # 六维/十维研究汇总
│   ├── quality_check.py          # 通用/投资专项质检
│   └── investor_v2_audit.py      # 旧Skill迁移审计
...
**主理人（Zhuliren）** 是这个衍生版本的新名称。它延续原项目“从公开信息提炼认知框架”的方法，生成的不是人物复制品，而是一套可运行的思维视角。
...
原项目作者：**花叔 Huashu**
当前版本维护者：**[kenter1643](https://github.com/kenter1643)**
...
MIT License。原始版权归 [花叔 Huashu](https://github.com/alchaincyf) 所有，当前衍生版本由 [kenter1643](https://github.com/kenter1643) 维护。
...
**[colleague-skill](https://github.com/titanwings/colleague-skill)** proved that distilling a person into an AI skill is viable. **Zhuliren** asks: why stop at colleagues? Distill the best minds in every field — Munger, Feynman, Musk, Naval — people who conveniently left mountains of distillable material behind.

Zhuliren is an [Agent Skill](https://agentskills.io) that extracts cognitive frameworks — mental models, decision heuristics, expression DNA — from any public figure into a runnable perspective skill. Works in Claude Code, Codex, Cursor, OpenClaw, Hermes Agent, CodeBuddy, Workbuddy, Gemini CLI, OpenCode, and 50+ skills-compatible runtimes.

Not role-playing. Cognitive architecture extraction.

**Install** (cross-runtime, auto-detects your agent): `npx skills add kenter1643/skills --skill zhuliren-skill`
